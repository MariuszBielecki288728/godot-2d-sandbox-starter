...
`WorldState.is_exposed_to_sky(tile)` applies the starter shelter rule: a tile is exposed only when there is no solid tile vertically above it. `WeatherView` draws rain only from the visible top down to the first solid column obstruction; it does not create physics objects for rain.
