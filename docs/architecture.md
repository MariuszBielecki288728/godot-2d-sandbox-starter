...
`WorldState.is_exposed_to_sky(tile)` applies the starter shelter rule: a tile is exposed only when there is no solid tile vertically above it. `WeatherView` observes `WorldState.tile_changed`, reconnecting safely whenever a loaded world replaces the old one. It samples visible world tile columns at their canonical centers, converts segments to screen space, and clips each final streak at the first solid column obstruction; it does not create physics objects for rain.
